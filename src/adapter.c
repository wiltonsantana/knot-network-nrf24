...
static bool offline_foreach(const void *key, void *value, void *user_data)
{
	...
	hal_log_info("ctime: %d", ctime);

	if (device_is_paired(device))
		return false;

	if (hal_timeout(ctime, device_get_last_seen(value), 5000) > 0) {
		device_get_address(device, &addr);
...
static void mgmt_timeout_cb(struct l_timeout *timeout, void *user_data)
{
	...
	hal_log_info("Timestamp: %d", timestamp);
	nremoved = l_hashmap_foreach_remove(adapter.offline_list,
					    offline_foreach,
					    L_UINT_TO_PTR(timestamp));
	hal_log_info("%d devices were removed from offline list", nremoved);

	l_timeout_modify(mgmt_timeout, 10);
}
...
static int radio_init(uint8_t channel, const struct nrf24_mac *addr)
{
	...
	mgmt_timeout = l_timeout_create(10, mgmt_timeout_cb, NULL, NULL);
