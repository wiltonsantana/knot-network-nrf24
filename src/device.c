...
static void emit_signal_string(const char *path, const char *prop,
			       const char *value)
{
	struct l_dbus_message *signal;
	struct l_dbus_message_builder *builder;

	signal = l_dbus_message_new_signal(dbus_get_bus(), path,
					 ...
					 void *user_data)
{
	...
	emit_signal_string(device->dpath, "Name", device->name);
